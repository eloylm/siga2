<?php
     ...
     * listing a collection of Muestras objects.  It includes
     ...
     *
     */
    class MuestrasDataGridGen extends QFilteredDataGrid {
        /**
        ...
        * way of adding a column to this Muestras datagrid.  The description of a column's
        * content can be either a text string description of a simple field name
        * in the Muestras object, or it can be any QQNode extending from QQN::Muestras().
        *
        ...
        *
         ...
         * sorting using two fields from the Muestras object.
         *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Muestras
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intMuestrasId=<?=urlencode($_ITEM->MuestrasId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->MuestrasId)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->MuestrasId, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
       ...
       public function MetaDataBinder() {
           // Remember!  We need to first set the TotalItemCount, which will affect the calcuation of LimitClause below
            if ($this->Paginator) {
                $this->TotalItemCount = Muestras::QueryCount($this->Conditions);
            }
            ...
            // Set the DataSource to be a Query result from Muestras, given the clauses above
            $this->DataSource = Muestras::QueryArray($this->Conditions, $objClauses);
        }
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Muestras-based QQNode.
		 * It will also verify that it is a proper Muestras-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'muestras') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "muestras".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'MuestrasId': return QQN::Muestras()->MuestrasId;
				case 'Detalle': return QQN::Muestras()->Detalle;
				case 'Observaciones': return QQN::Muestras()->Observaciones;
				case 'ClientesId': return QQN::Muestras()->ClientesId;
				case 'Clientes': return QQN::Muestras()->Clientes;
				case 'Estado': return QQN::Muestras()->Estado;
				case 'UsuariosId': return QQN::Muestras()->UsuariosId;
				case 'Usuarios': return QQN::Muestras()->Usuarios;
				default: throw new QCallerException('Simple Property not found in MuestrasDataGrid content: ' . $mixContent);
