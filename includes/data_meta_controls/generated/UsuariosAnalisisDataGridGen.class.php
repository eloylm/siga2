<?php
     ...
     * listing a collection of UsuariosAnalisis objects.  It includes
     ...
     *
     */
    class UsuariosAnalisisDataGridGen extends QFilteredDataGrid {
        /**
        ...
        * way of adding a column to this UsuariosAnalisis datagrid.  The description of a column's
        * content can be either a text string description of a simple field name
        * in the UsuariosAnalisis object, or it can be any QQNode extending from QQN::UsuariosAnalisis().
        *
        ...
        *
         ...
         * sorting using two fields from the UsuariosAnalisis object.
         *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from UsuariosAnalisis
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intUsuariosAnalisisId=<?=urlencode($_ITEM->UsuariosAnalisisId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->UsuariosAnalisisId)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->UsuariosAnalisisId, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
       ...
       public function MetaDataBinder() {
           // Remember!  We need to first set the TotalItemCount, which will affect the calcuation of LimitClause below
            if ($this->Paginator) {
                $this->TotalItemCount = UsuariosAnalisis::QueryCount($this->Conditions);
            }
            ...
            // Set the DataSource to be a Query result from UsuariosAnalisis, given the clauses above
            $this->DataSource = UsuariosAnalisis::QueryArray($this->Conditions, $objClauses);
        }
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a UsuariosAnalisis-based QQNode.
		 * It will also verify that it is a proper UsuariosAnalisis-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'usuarios_analisis') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "usuarios_analisis".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'UsuariosId': return QQN::UsuariosAnalisis()->UsuariosId;
				case 'Usuarios': return QQN::UsuariosAnalisis()->Usuarios;
				case 'AnalisisId': return QQN::UsuariosAnalisis()->AnalisisId;
				case 'Analisis': return QQN::UsuariosAnalisis()->Analisis;
				case 'UsuariosAnalisisId': return QQN::UsuariosAnalisis()->UsuariosAnalisisId;
				default: throw new QCallerException('Simple Property not found in UsuariosAnalisisDataGrid content: ' . $mixContent);
