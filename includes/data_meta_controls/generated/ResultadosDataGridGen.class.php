<?php
     ...
     * listing a collection of Resultados objects.  It includes
     ...
     *
     */
    class ResultadosDataGridGen extends QFilteredDataGrid {
        /**
        ...
        * way of adding a column to this Resultados datagrid.  The description of a column's
        * content can be either a text string description of a simple field name
        * in the Resultados object, or it can be any QQNode extending from QQN::Resultados().
        *
        ...
        *
         ...
         * sorting using two fields from the Resultados object.
         *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Resultados
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intResultadosId=<?=urlencode($_ITEM->ResultadosId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->ResultadosId)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->ResultadosId, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
       ...
       public function MetaDataBinder() {
           // Remember!  We need to first set the TotalItemCount, which will affect the calcuation of LimitClause below
            if ($this->Paginator) {
                $this->TotalItemCount = Resultados::QueryCount($this->Conditions);
            }
            ...
            // Set the DataSource to be a Query result from Resultados, given the clauses above
            $this->DataSource = Resultados::QueryArray($this->Conditions, $objClauses);
        }
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Resultados-based QQNode.
		 * It will also verify that it is a proper Resultados-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'resultados') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "resultados".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'ResultadosId': return QQN::Resultados()->ResultadosId;
				case 'OrdenesTrabajoId': return QQN::Resultados()->OrdenesTrabajoId;
				case 'OrdenesTrabajo': return QQN::Resultados()->OrdenesTrabajo;
				case 'AnalisisId': return QQN::Resultados()->AnalisisId;
				case 'Analisis': return QQN::Resultados()->Analisis;
				case 'Estado': return QQN::Resultados()->Estado;
				case 'UsuariosId': return QQN::Resultados()->UsuariosId;
				case 'Usuarios': return QQN::Resultados()->Usuarios;
				case 'Fecha': return QQN::Resultados()->Fecha;
				case 'Norma': return QQN::Resultados()->Norma;
				case 'Medicion': return QQN::Resultados()->Medicion;
				case 'Resultado': return QQN::Resultados()->Resultado;
				default: throw new QCallerException('Simple Property not found in ResultadosDataGrid content: ' . $mixContent);
