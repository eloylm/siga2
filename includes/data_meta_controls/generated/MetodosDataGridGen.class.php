<?php
     ...
     * listing a collection of Metodos objects.  It includes
     ...
     *
     */
    class MetodosDataGridGen extends QFilteredDataGrid {
        /**
        ...
        * way of adding a column to this Metodos datagrid.  The description of a column's
        * content can be either a text string description of a simple field name
        * in the Metodos object, or it can be any QQNode extending from QQN::Metodos().
        *
        ...
        *
         ...
         * sorting using two fields from the Metodos object.
         *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Metodos
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intMetodoId=<?=urlencode($_ITEM->MetodoId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->MetodoId)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->MetodoId, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
       ...
       public function MetaDataBinder() {
           // Remember!  We need to first set the TotalItemCount, which will affect the calcuation of LimitClause below
            if ($this->Paginator) {
                $this->TotalItemCount = Metodos::QueryCount($this->Conditions);
            }
            ...
            // Set the DataSource to be a Query result from Metodos, given the clauses above
            $this->DataSource = Metodos::QueryArray($this->Conditions, $objClauses);
        }
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Metodos-based QQNode.
		 * It will also verify that it is a proper Metodos-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'metodos') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "metodos".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'MetodoId': return QQN::Metodos()->MetodoId;
				case 'Nombre': return QQN::Metodos()->Nombre;
				default: throw new QCallerException('Simple Property not found in MetodosDataGrid content: ' . $mixContent);
