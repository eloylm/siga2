<?php
     ...
     * listing a collection of Unidades objects.  It includes
     ...
     *
     */
    class UnidadesDataGridGen extends QFilteredDataGrid {
        /**
        ...
        * way of adding a column to this Unidades datagrid.  The description of a column's
        * content can be either a text string description of a simple field name
        * in the Unidades object, or it can be any QQNode extending from QQN::Unidades().
        *
        ...
        *
         ...
         * sorting using two fields from the Unidades object.
         *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Unidades
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intUnidadId=<?=urlencode($_ITEM->UnidadId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->UnidadId)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->UnidadId, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
       ...
       public function MetaDataBinder() {
           // Remember!  We need to first set the TotalItemCount, which will affect the calcuation of LimitClause below
            if ($this->Paginator) {
                $this->TotalItemCount = Unidades::QueryCount($this->Conditions);
            }
            ...
            // Set the DataSource to be a Query result from Unidades, given the clauses above
            $this->DataSource = Unidades::QueryArray($this->Conditions, $objClauses);
        }
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Unidades-based QQNode.
		 * It will also verify that it is a proper Unidades-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'unidades') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "unidades".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'UnidadId': return QQN::Unidades()->UnidadId;
				case 'Nombre': return QQN::Unidades()->Nombre;
				default: throw new QCallerException('Simple Property not found in UnidadesDataGrid content: ' . $mixContent);
