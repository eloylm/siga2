<?php
     ...
     * listing a collection of Analisis objects.  It includes
     ...
     *
     */
    class AnalisisDataGridGen extends QFilteredDataGrid {
        /**
        ...
        * way of adding a column to this Analisis datagrid.  The description of a column's
        * content can be either a text string description of a simple field name
        * in the Analisis object, or it can be any QQNode extending from QQN::Analisis().
        *
        ...
        *
         ...
         * sorting using two fields from the Analisis object.
         *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Analisis
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intAnalisisId=<?=urlencode($_ITEM->AnalisisId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->AnalisisId)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->AnalisisId, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
       ...
       public function MetaDataBinder() {
           // Remember!  We need to first set the TotalItemCount, which will affect the calcuation of LimitClause below
            if ($this->Paginator) {
                $this->TotalItemCount = Analisis::QueryCount($this->Conditions);
            }
            ...
            // Set the DataSource to be a Query result from Analisis, given the clauses above
            $this->DataSource = Analisis::QueryArray($this->Conditions, $objClauses);
        }
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Analisis-based QQNode.
		 * It will also verify that it is a proper Analisis-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'analisis') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "analisis".');
