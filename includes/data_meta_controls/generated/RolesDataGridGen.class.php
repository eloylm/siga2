<?php
     ...
     * listing a collection of Roles objects.  It includes
     ...
     *
     */
    class RolesDataGridGen extends QFilteredDataGrid {
        /**
        ...
        * way of adding a column to this Roles datagrid.  The description of a column's
        * content can be either a text string description of a simple field name
        * in the Roles object, or it can be any QQNode extending from QQN::Roles().
        *
        ...
        *
         ...
         * sorting using two fields from the Roles object.
         *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Roles
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intRolesId=<?=urlencode($_ITEM->RolesId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->RolesId)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->RolesId, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
       ...
       public function MetaDataBinder() {
           // Remember!  We need to first set the TotalItemCount, which will affect the calcuation of LimitClause below
            if ($this->Paginator) {
                $this->TotalItemCount = Roles::QueryCount($this->Conditions);
            }
            ...
            // Set the DataSource to be a Query result from Roles, given the clauses above
            $this->DataSource = Roles::QueryArray($this->Conditions, $objClauses);
        }
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Roles-based QQNode.
		 * It will also verify that it is a proper Roles-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'roles') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "roles".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'RolesId': return QQN::Roles()->RolesId;
				case 'Descrip': return QQN::Roles()->Descrip;
				case 'Comentario': return QQN::Roles()->Comentario;
				default: throw new QCallerException('Simple Property not found in RolesDataGrid content: ' . $mixContent);
