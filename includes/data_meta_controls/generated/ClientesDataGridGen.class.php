<?php
     ...
     * listing a collection of Clientes objects.  It includes
     ...
     *
     */
    class ClientesDataGridGen extends QFilteredDataGrid {
        /**
        ...
        * way of adding a column to this Clientes datagrid.  The description of a column's
        * content can be either a text string description of a simple field name
        * in the Clientes object, or it can be any QQNode extending from QQN::Clientes().
        *
        ...
        *
         ...
         * sorting using two fields from the Clientes object.
         *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Clientes
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intClientesId=<?=urlencode($_ITEM->ClientesId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->ClientesId)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->ClientesId, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
       ...
       public function MetaDataBinder() {
           // Remember!  We need to first set the TotalItemCount, which will affect the calcuation of LimitClause below
            if ($this->Paginator) {
                $this->TotalItemCount = Clientes::QueryCount($this->Conditions);
            }
            ...
            // Set the DataSource to be a Query result from Clientes, given the clauses above
            $this->DataSource = Clientes::QueryArray($this->Conditions, $objClauses);
        }
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Clientes-based QQNode.
		 * It will also verify that it is a proper Clientes-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'clientes') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "clientes".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'ClientesId': return QQN::Clientes()->ClientesId;
				case 'Nombre': return QQN::Clientes()->Nombre;
				case 'Direccion': return QQN::Clientes()->Direccion;
				case 'Localidad': return QQN::Clientes()->Localidad;
				case 'Provincia': return QQN::Clientes()->Provincia;
				case 'Email': return QQN::Clientes()->Email;
				case 'ResponsableContacto': return QQN::Clientes()->ResponsableContacto;
				case 'FechaAlta': return QQN::Clientes()->FechaAlta;
				case 'Estado': return QQN::Clientes()->Estado;
				case 'EmailSecundario': return QQN::Clientes()->EmailSecundario;
				case 'Telefono': return QQN::Clientes()->Telefono;
				default: throw new QCallerException('Simple Property not found in ClientesDataGrid content: ' . $mixContent);
