<?php
     ...
     * listing a collection of Usuarios objects.  It includes
     ...
     *
     */
    class UsuariosDataGridGen extends QFilteredDataGrid {
        /**
        ...
        * way of adding a column to this Usuarios datagrid.  The description of a column's
        * content can be either a text string description of a simple field name
        * in the Usuarios object, or it can be any QQNode extending from QQN::Usuarios().
        *
        ...
        *
         ...
         * sorting using two fields from the Usuarios object.
         *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Usuarios
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intUsuariosId=<?=urlencode($_ITEM->UsuariosId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->UsuariosId)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->UsuariosId, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
       ...
       public function MetaDataBinder() {
           // Remember!  We need to first set the TotalItemCount, which will affect the calcuation of LimitClause below
            if ($this->Paginator) {
                $this->TotalItemCount = Usuarios::QueryCount($this->Conditions);
            }
            ...
            // Set the DataSource to be a Query result from Usuarios, given the clauses above
            $this->DataSource = Usuarios::QueryArray($this->Conditions, $objClauses);
        }
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Usuarios-based QQNode.
		 * It will also verify that it is a proper Usuarios-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'usuarios') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "usuarios".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'UsuariosId': return QQN::Usuarios()->UsuariosId;
				case 'Usuario': return QQN::Usuarios()->Usuario;
				case 'Nombre': return QQN::Usuarios()->Nombre;
				case 'Email': return QQN::Usuarios()->Email;
				case 'Habilitado': return QQN::Usuarios()->Habilitado;
				case 'Password': return QQN::Usuarios()->Password;
				default: throw new QCallerException('Simple Property not found in UsuariosDataGrid content: ' . $mixContent);
