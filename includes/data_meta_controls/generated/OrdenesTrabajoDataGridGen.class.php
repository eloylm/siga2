<?php
     ...
     * listing a collection of OrdenesTrabajo objects.  It includes
     ...
     *
     */
    class OrdenesTrabajoDataGridGen extends QFilteredDataGrid {
        /**
        ...
        * way of adding a column to this OrdenesTrabajo datagrid.  The description of a column's
        * content can be either a text string description of a simple field name
        * in the OrdenesTrabajo object, or it can be any QQNode extending from QQN::OrdenesTrabajo().
        *
        ...
        *
         ...
         * sorting using two fields from the OrdenesTrabajo object.
         *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from OrdenesTrabajo
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->OrdenesTrabajoId, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
       ...
       public function MetaDataBinder() {
           // Remember!  We need to first set the TotalItemCount, which will affect the calcuation of LimitClause below
            if ($this->Paginator) {
                $this->TotalItemCount = OrdenesTrabajo::QueryCount($this->Conditions);
            }
            ...
            $this->DataSource = OrdenesTrabajo::QueryArray($this->Conditions, $objClauses);
        }
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a OrdenesTrabajo-based QQNode.
		 * It will also verify that it is a proper OrdenesTrabajo-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'ordenes_trabajo') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "ordenes_trabajo".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'OrdenesTrabajoId': return QQN::OrdenesTrabajo()->OrdenesTrabajoId;
				case 'FechaEntrada': return QQN::OrdenesTrabajo()->FechaEntrada;
				case 'ClientesId': return QQN::OrdenesTrabajo()->ClientesId;
				case 'Clientes': return QQN::OrdenesTrabajo()->Clientes;
				case 'Cargador': return QQN::OrdenesTrabajo()->Cargador;
				case 'Buque': return QQN::OrdenesTrabajo()->Buque;
				case 'Kg': return QQN::OrdenesTrabajo()->Kg;
				case 'Puerto': return QQN::OrdenesTrabajo()->Puerto;
				case 'ReferenciaCliente': return QQN::OrdenesTrabajo()->ReferenciaCliente;
				case 'Destino': return QQN::OrdenesTrabajo()->Destino;
				case 'MuestrasId': return QQN::OrdenesTrabajo()->MuestrasId;
				case 'Muestras': return QQN::OrdenesTrabajo()->Muestras;
				case 'UsuariosId': return QQN::OrdenesTrabajo()->UsuariosId;
				case 'Usuarios': return QQN::OrdenesTrabajo()->Usuarios;
				case 'New': return QQN::OrdenesTrabajo()->New;
				case 'Estado': return QQN::OrdenesTrabajo()->Estado;
				case 'Observaciones': return QQN::OrdenesTrabajo()->Observaciones;
				case 'FechaFinalizado': return QQN::OrdenesTrabajo()->FechaFinalizado;
				default: throw new QCallerException('Simple Property not found in OrdenesTrabajoDataGrid content: ' . $mixContent);
